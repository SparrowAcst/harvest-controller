...
const normalizeVersion = version => {
    if(!version) return
    keys(version).forEach( key => {
        if(!DEFAULT_FIELDS.includes(key)) delete version[key]
    })
    return version
}
...
const resolveData = async (options = {}) => {
    ...
    try {
        ...
        let data = await mongodb.aggregate({
            comment:"resolve data for dataView",
            db,
...
const Worker = class {
    ...
    async branch(options = {}) {
        ...
        try {
            ...
            source.forEach(s => {
                ...
                parent = normalizeVersion(parent)    
    ...
    async save(options = {}) {
        ...
        try {
            ...
            let newVersion = normalizeVersion({
                id: uuid(),
                dataId,
                task: parent.task,
                user,
    ...
    async submit(options = {}) {
        ...
        try {
            ...
            freezePeriod = freezePeriod || options.freezePeriod || EXPIRATION_PERIOD || [1, "minutes"]
    ...
    async commit(options = {}) {
        ...
        try {
            ...
            let newMainHead = normalizeVersion({
                id: uuid(),
                dataId,
                task: parent.task,
                prev: [{
    ...
    async merge(options = {}) {
        ...
        try {
            ...
            parents = parents.map( p => normalizeVersion(p))
            ...
            let mergeHead = normalizeVersion({
                id: uuid(),
                dataId,
                task: parents[0].task,
                user,
    ...
    getHistory(options = {}) {
        ...
        let step = 1
        while (f && (step < maxDepth) && ((stopAt && isFunction(stopAt)) ? !stopAt(current) : true)) {
            res.push(f)
            current = f
            step++
    ...
    getGraph(options = {}) {
        ...
        let versions = cache[dataId].map(d => {
            d.name = (d.type == "branch") ? d.metadata.actual_task : d.id
...
const createWorker = async options => {
    ...
    let data = await mongodb.aggregate({
        comment:`init data brancher for ${dataId}`,
        db,
        collection: `${db.name}.savepoints`,
        pipeline: [{
                $match: {
