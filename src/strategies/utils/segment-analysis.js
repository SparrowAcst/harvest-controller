...
const getBreathRate = sa => {
    ...
    let segments = sa.segmentation
    segments = sortBy(segments, s => s.start).filter(s => s.type == "Inhale")
    let res = []
...
const getDepricated = sa => {
    ...
    let deprecated = segments.filter(s => s.type == "unsegmentable").map(s => s.end - s.start).reduce((a, v) => a + v, 0)
    let duration = last(segments).end
...
const getDuration = sa => {
    ...
    let segments = select(sa)
    return last(segments).end

}

const getSystoleDiastole = sa => {
    
    let cycles = sa.cardioCycles
    ...
    cycles.forEach( cycle  => {
        let edge = find(cycle, s => s.type == "S2")
        let start = first(cycle).start
        let end = last(cycle).start
        edge = edge.start
        systole.push({
            type:"systole",
            start: Number.parseFloat((start).toFixed(4)),
            end: Number.parseFloat((edge).toFixed(4)),
            duration: Number.parseFloat((edge - start).toFixed(4))
        })
        diastole.push({
            type:"diastole",
            start: Number.parseFloat((edge).toFixed(4)),
            end: Number.parseFloat((end).toFixed(4)),
            duration: Number.parseFloat((end - edge).toFixed(4))
        })
    })
...
const getTotals = sa => {
    ...
    let sysdia = getSystoleDiastole(res)
...
const getSystoleDiastoleBars = sa => {
    ...
    // let systoleAvg = avg(systole) //.map( d => d.duration)) //reduce((a, b) => a + b, 0) / systole.length
    // let systoleStd = std(systole)
    // let cc = systole.map((s, index) => s + diastole[index])
    // let ccMax = max(cc) //cc.reduce((a, c) => (a > c) ? a : c)
    // let diastoleAvg = avg(cc) //.reduce((a, b) => a + b, 0) / diastole.length
    // let diastoleStd = std(cc)
...
const getSystoleDiastoleScatterPlot = sa => {
    ...
    return {
          ...
          series: [
            ...
            //     },
            //     data: [
            //       [{ coord: [0.3,0.3],symbol:"none" }, { coord: [1.7, 1.7],symbol:"none" }]
            //     ]  
            //   }
            }
          ]
...
const getS4S1interval = sa => {
    ...
    let pool = sortBy(sa.segmentation.filter(s => (["S4", "S1", "unsegmentable"]).includes(s.type)), d => d.start)
    let res = []
...
const getS2S3interval = sa => {
    ...
    let pool = sortBy(sa.segmentation.filter(s => (["S2", "S3", "unsegmentable"]).includes(s.type)), d => d.start)
    let res = []
    for (let i = 0; i < pool.length - 1; i++) {
...
const getSegmentDurationBoxplot = sa => {
    ...
    let pool = sa.segmentation.filter(s => !(["systole", "diastole"]).includes(s.type))

    pool = pool.concat(sa.systole).concat(sa.diastole).concat(getS4S1interval(sa)).concat(getS2S3interval(sa))
...
const getHFBoxplot = sa => {
    ...
    let pool = sa.segmentation.filter(s => (["S3", "S4"]).includes(s.type)).map(d => ({
        type: d.type,
        hf: d.hf
    }))
...
const getPoincareChart = sa => {
    ...
    return {
        ...
        series: [{
            ...
            //     },
            //     data: [
            //         [{ coord: [min_, min], symbol: "none" }, { coord: [max, max], symbol: "none" }]
            //     ]
            // }
        }]
    }
...
const getSegmentationAnalysis = segmentation => {
    ...
    let sa = {
        segmentation: parse(segmentation)
    }
