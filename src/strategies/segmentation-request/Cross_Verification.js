...
const openRequest = async options => {
    ...
    let altVersions = await controller.selectTask({
        matchVersion: v =>
            version.metadata.task.Cross_Verification.versions.includes(v.id)
    })
...
const updateRequest = async options => {
    ...
    console.log(`>> Cross_Verification: UPDATE REQUEST ${options.requestId}: START`)
    ...
    let v = await brancher.save({
        ...
        data,
        metadata: {
            "task.Cross_Verification.status": "in progress",
    ...
    console.log(`>> Cross_Verification: UPDATE REQUEST ${options.requestId}: DONE`)
